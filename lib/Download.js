...
class Download extends EventEmitter
{
     ...
     * @param {Function|null} postProcess - Function to execute before jobEnd event
     */
    constructor({url, file, hash = null, postProcess = null}) {
        super();
     ...
     * @private
     */
    async onEnd(err = null) {
        this.dispose();

        // Check file size
        if (!err && this._size > 0) {
            const size = await fileSize(this._file);
        ...
        // Check file hash
        if (!err && this._hash) {
            const hash = await fileHash(this._file);
        ...
        if (!err && this._postProcess) {
            try {
                await this._postProcess(this.descriptor);
            } catch (e) {
                err = e;
            }
        }
        ...
        if (err) {
            try {
                await pify(fs.unlink)(this._file);
