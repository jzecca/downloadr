...
class Download extends EventEmitter
{
     ...
     */
    constructor({url, file, hash = null, data = null}) {
        super();
        ...
        this._stream = got.stream(this._url, options)
            ...
            .on('response', response => {
                ...
                this._progressStream = progress({ length: this._size, time: 500 })
                    .on('progress', progress => this.emit('progress', progress.percentage / 100));
     ...
     *
     * @return {{url: string, file: string, hash: (string|null), data: (Object|null)}}
     */
    get descriptor() {
        return {
            url:  this._url,
